...
public class Ticket {
    ...
    from.openPrivateChannel()
            ...
            c ->
                c.sendMessage(
                    TicketMessage.from(embeds.proposeOpening(sentEmote.getAsMention()))
                        ...
                        .addOption(
                            "Proposer une amélioration", "op-enhancement", Emoji.fromUnicode("✨"))
        ...
        .queue(
            ...
            new ErrorHandler()
                ...
                .handle(
                    ErrorResponse.CANNOT_SEND_TO_USER, exception -> handleUnableToDmUser(channel)));
  }
  ...
  public void forceOpening(User by, MessageChannel srcChannel) {
    TicketMessage.from(embeds.forceOpening(sentEmote.getAsMention()))
        ...
        .queue(
            ...
            new ErrorHandler()
                .handle(ErrorResponse.UNKNOWN_MESSAGE, unused -> openOnServer(true, by, null))
                .handle(
                    ErrorResponse.CANNOT_SEND_TO_USER, unused -> handleUnableToDmUser(srcChannel)));
  }
  ...
  private void openChannel(boolean forced, User by, String reason, TextChannel channel) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Unable to retrieve webhook for ticket #" + channel.getName() + " !", e);
    }
        ...
        .queue(
            webhooks -> {
              try {
                    ...
                    switch (webhooks.size()) {
                      case 0 -> getQueue(
                          to.createWebhook(from.getName())
                              .setAvatar(
                                  Icon.from(new URL(from.getEffectiveAvatarUrl()).openStream())));
                ...
                throw new RuntimeException(
                    "Unable to retrieve webhook for ticket #" + to.getName() + " !", e);
              }
            });
